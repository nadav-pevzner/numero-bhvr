...
export const statusEnum = pgEnum("status", ["active", "solved", "failed", "partial"]);
...
export const questions = pgTable(
  ...
  (table) => [
    {
      conversationIdIdx: index("questions_conversation_id_idx").on(table.conversationId),
      userStatusIdx: index("questions_user_id_status_idx").on(table.userId, table.status),
    },
  ],
);
...
export const mainTopics = pgTable(
  ...
  (table) => [
    {
      gradeNameUnique: uniqueIndex("main_topics_grade_id_name_key").on(table.gradeId, table.name),
      gradeIdx: index("idx_main_topics_grade").on(table.gradeId),
      orderIdx: index("idx_main_topics_order").on(table.gradeId, table.displayOrder),
    },
  ],
);
  ...
  (table) => [
    {
      ...
      orderIdx: index("idx_subtopics_order").on(table.mainTopicId, table.displayOrder),
    },
  ],
);
